...
本次更新主要集中於提升前端隨機抽取功能的使用者體驗 (UX)。

### 1. 引入「感知隨機性抽取」

為了改善純粹數學隨機性可能導致的內容重複問題（即連續或短期內重複抽到同一首歌），我們調整了抽取邏輯，以優化使用者對結果的「感知」。

* **核心機制：** 系統現在會維護一個包含最近 **10 個** 抽中歌名 (Song Title) 的歷史緩存。
* **優化目標：** 確保在短時間內，使用者不會重複看到同一首歌曲的歌詞，無論該歌曲在資料庫中擁有多少筆記錄。
* **技術實作：** 在 JavaScript 的抽取函式中，使用 `do...while` 循環進行檢查，動態排除所有與歷史緩存中歌名相同的歌詞記錄。

### 2. 程式碼架構調整

以下是針對前端 `index.html` 文件中的 JavaScript 進行的調整：

* 新增了 `HISTORY_SIZE = 10` 和 `historyTitles` 陣列，用於實現歷史記錄緩存。
* 修改了核心抽取邏輯，導入了檢查和排除機制。
